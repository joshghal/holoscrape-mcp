...
const TOOLS = [
  ...
  T('tab_here',
    ...
    { tabId: N('The tab to move. It keeps its id — this is the same tab, somewhere else.'),
      ...
      network: S('START WITH "*" ON ANY SITE YOU DO NOT ALREADY KNOW. That is DISCOVERY: it maps '
        ...
        + 'is attached for the duration of the page load and every matching JSON response body is '
  ...
  T('page_state',
    ...
    + 'call, "@net(*)" watches every XHR and fetch, "@net()" polls what has arrived SINCE THE LAST '
    + 'POLL, and "@net(stop)" ends it. Use it when the data appears because of something you are '
    + 'about to do — a filter click, a drawer, a search box, a scroll — rather than on page load: '
    + 'start the watch FIRST, then do the thing with any tool, then poll. A watch cannot recover '
    + 'requests the page already made, so starting one after the fact returns nothing. Each poll '
    + 'reports the responses and the $. paths inside them, never the bodies; hand those paths to '
    + 'page_harvest({network}) or page_grow({network, rows}). Holds a debugger on the tab (yellow '
    + 'bar, and DevTools cannot be open on it) until stopped, so stop when done.\n'
